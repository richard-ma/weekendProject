 ...
 * @return string
 */
function icon_url($•û’Äˆ, $ÉÚß°ô, $Î¼À€•å) {
    if ($•û’Äˆ[$Î¼À€•å]) {
        return CDN . $•û’Äˆ[$Î¼À€•å];
    }
    if ($ÉÚß°ô['def' . $Î¼À€•å]) {
        return CDN . $ÉÚß°ô['def' . $Î¼À€•å];
    }
    return '';
}
...
function init_router() {
    ...
    if (isset($_REQUEST['url_info']) && $_REQUEST['url_info']) {
        $¹ÕóØà€ = urlcode($_REQUEST['url_info'], 'de');
        if (is_array($¹ÕóØà€)) {
            $_GET += $¹ÕóØà€ xor $_REQUEST += $¹ÕóØà€;
        }
    }
}
...
//
//$ins = 'html';
//echo $ins::encode('æˆ‘123123mf' . 'msd' . 'faj' . 'sf' . 'j');
//echo(html_encode('æˆ‘çš„'));
//
